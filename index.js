...
async function init() {
  ...
  let artifacts = []
  console.log(`[INFO] Getting information for: ${targetedApps.length} apps`)
  for (const app of targetedApps) {
    const { app_slug, name } = app
    console.log(`[INFO] Getting information for: ${name}`)
    const { data: { data: appData } = {} } = await axios.get(getBuildsUrl({ app_slug })).catch(ex => console.log(ex))
    console.log(`[INFO] Filtering by [workflowFilter: ${!!workflowFilter}, statusFilter: ${!!statusFilter}]`)
    ...
    console.log(`[INFO] Latest build_slug matching filter parameters: ${latestBuild?.slug || 'NO BUILD FOUND'}`)
    const { slug: build_slug, finished_at: build_finished_at } = latestBuild
    console.log(`[INFO] Getting artifacts for build`)
    ...
    console.log(`[INFO] Getting artifact information`)
  ...
  if (publicInstallPages.warning) {
    console.log(`\x1b[43m\x1b[30m[WARN]\x1b[0m There are potential issues with the builds check: './data/${date}_app_information.json' for more information`)
  }
